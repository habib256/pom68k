...
**Generated** by `tools/changelog_index.py` from the 457 dated entries in `CHANGELOG.md`. Do not edit by hand: regenerate. `docs_test` § 7 only compares the entry COUNT, so a stale hook or a wrong anchor here passes it — regenerate after every CHANGELOG edit.
...
| [Storage — SCSI, IWM, SWIM, media](#storage--scsi-iwm-swim-media) | 52 |
...
- **2026-09-08** — [Floppy 1.44 MB: the SuperDrive patch's dispatchers all pass through because the mount read is dispatched direct, not via the .Sony async wrapper](CHANGELOG.md#2026-09-08-floppy-ism-vs-iwm)
