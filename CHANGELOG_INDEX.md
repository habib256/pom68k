...
**Generated** by `tools/changelog_index.py` from the 468 dated entries in `CHANGELOG.md`. Do not edit by hand: regenerate. `docs_test` § 7 only compares the entry COUNT, so a stale hook or a wrong anchor here passes it — regenerate after every CHANGELOG edit.
...
| [Storage — SCSI, IWM, SWIM, media](#storage--scsi-iwm-swim-media) | 63 |
...
- **2026-09-08** — [Floppy 1.44 MB: THE ROOT -- the drive reads in GCR mode (mfmMode_=0) on an MFM disk; forcing MFM makes Prime #1 succeed](CHANGELOG.md#2026-09-08-floppy-ism-vs-iwm)
