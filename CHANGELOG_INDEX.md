...
**Generated** by `tools/changelog_index.py` from the 456 dated entries in `CHANGELOG.md`. Do not edit by hand: regenerate. `docs_test` § 7 only compares the entry COUNT, so a stale hook or a wrong anchor here passes it — regenerate after every CHANGELOG edit.
...
| [Storage — SCSI, IWM, SWIM, media](#storage--scsi-iwm-swim-media) | 51 |
...
- **2026-09-08** — [Floppy 1.44 MB: the ROM cannot do MFM at all; the read reaches the SuperDrive patch on the wrong (synchronous) frame and is declined](CHANGELOG.md#2026-09-08-floppy-ism-vs-iwm)
