...
**Generated** by `tools/changelog_index.py` from the 588 dated entries in `CHANGELOG.md`. Do not edit by hand: regenerate. `docs_test` § 7 only compares the entry COUNT, so a stale hook or a wrong anchor here passes it — regenerate after every CHANGELOG edit.
...
- **2026-08-31 (seventeenth)** — [Speedometer's `BSR.L` becomes native through an exact three-word 68030 JIT fetch proof](CHANGELOG.md#2026-08-31-speedometer-bsr-long)
...
- **2026-09-08** — [Floppy 1.44 MB: the ISM read engine and SWIM2 both work; the failure is specific to the LC II SWIM1/ISM driver dispatch](CHANGELOG.md#2026-09-08--floppy-144-mb-the-ism-read-engine-and-swim2-both-work-the-failure-is-specific-to-the-lc-ii-swim1ism-driver-dispatch)
- **2026-09-08** — [Floppy 1.44 MB: the failure is a two-phase GCR-then-MFM sequence; the MFM retry aborts on a drive-not-enabled status read](CHANGELOG.md#2026-09-08--floppy-144-mb-the-failure-is-a-two-phase-gcr-then-mfm-sequence-the-mfm-retry-aborts-on-a-drive-not-enabled-status-read)
- **2026-09-08** — [Floppy 1.44 MB: the SuperDrive patch's dispatchers all pass through because the mount read is dispatched direct, not via the .Sony async wrapper](CHANGELOG.md#2026-09-08--floppy-144-mb-the-superdrive-patchs-dispatchers-all-pass-through-because-the-mount-read-is-dispatched-direct-not-via-the-sony-async-wrapper)
- **2026-09-08** — [Floppy 1.44 MB: the ROM cannot do MFM at all; the read reaches the SuperDrive patch on the wrong (synchronous) frame and is declined](CHANGELOG.md#2026-09-08--floppy-144-mb-the-rom-cannot-do-mfm-at-all-the-read-reaches-the-superdrive-patch-on-the-wrong-synchronous-frame-and-is-declined)
- **2026-09-08** — [Floppy 1.44 MB: the abort is a driver/System control-flow bail before ACTION, not a device fault](CHANGELOG.md#2026-09-08--floppy-144-mb-the-abort-is-a-driversystem-control-flow-bail-before-action-not-a-device-fault)
...
- **2026-09-09** — [Floppy 1.44 MB: the fix reads a full valid MDB; the enable/motoron disconnect is NOT the mount blocker (it matches MAME)](CHANGELOG.md#2026-09-09-floppy-mdb-read)
...
- **2026-09-16 (nineteenth)** — [The relaunch is observed, not serialized: a smoke generation stages the DaynaPort card, re-executes, and the next generation sees the card](CHANGELOG.md#2026-09-16-relaunch-observed)
...
- **2026-09-17 (twenty-fourth)** — [The ATA driver is not a missing dump: it is inside Drive Setup, on volumes we already have. What blocks the IDE boot is an interrupt line](CHANGELOG.md#2026-09-17-ata-driver-not-missing)
- **2026-09-18 (eighth)** — [Mini vMac mounts a POM68K volume: the LToUDP interop goes both ways, and the hub's network number turns out to be asserted rather than learned](CHANGELOG.md#2026-09-18-minivmac-interop)
...
- **2026-09-02 (eleventh)** — [Two locked reference identities surface on the August backup: the "tree that never existed here" is found](CHANGELOG.md#2026-09-02-backup-harvest)
- **2026-09-02 (fifteenth)** — [The save-state path moves behind the slot's mutex: the nightly's TSan race was the smoke bypassing the channel](CHANGELOG.md#2026-09-02-savestate-path-mutex)
- **2026-09-02 (fourteenth)** — [The catalogue's density check stops forming a pointer, and the ASan nightly can compile again](CHANGELOG.md#2026-09-02-catalog-consteval)
- **2026-09-02 (ninth)** — [The dispatch cache and cold-block eviction land in the tree; the host froze before their tier, so the proof is owed](CHANGELOG.md#2026-09-02-dispatch-cache-lands)
- **2026-09-02 (seventeenth)** — [The nightly's first light: TSan proves the race fix, GCC-13 ASan compiles, and its first-ever run finds additive stacks](CHANGELOG.md#2026-09-02-nightly-first-light)
...
- **2026-09-16 (eighteenth)** — [The macii × 7.5.5 matrix cell is ruled PASS: six identical runs, Stickies in front and the Finder running](CHANGELOG.md#2026-09-16-macii-755-cell-ruled)
