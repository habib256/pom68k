...
**POM68K ships 34 machine profiles across 11 machine families. Every one boots
...
## 1. Done — the 34 profiles and the gate that proves each
...
|---|---|---|---|
| **AppleP IC IOP** (Apple 343S1021: 65C02 core + 2 KB shared RAM + 2 DMA channels + host/peripheral mailboxes + timer — MAME `machine/applepic.cpp`) **+ the OSS** interrupt controller | **Mac IIfx** — **LANDED 2026-08-01** as the 34th profile (`R65c02.*`, `ApplePic.*`, `IIfxMemory.*`; Finder on 7.6). **Quadra 900 / 950**: the Eclipse front end is in `Q700Memory`, the SWIM IOP's firmware still BRKs (`TODO.md` § 0) | IIfx `4147DD77`; Q900 shares `420DBFF3` with the Q700; Q950 `3DC27823` | ✅ / 🟡 |
