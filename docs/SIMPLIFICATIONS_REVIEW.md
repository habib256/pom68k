...
|---|---|---|---|
| F1 | "PRAM persistence missing on 4 platforms" | **WITHDRAWN — the finding was false.** All twelve platforms declare `loadPram`/`savePram` and every runner wires the pair (the per-platform `GuiRunner*.h`, first pair `GuiRunnerToby.h:54`/`:210`; the compacts at `PlatformCompact.cpp:156`/`GuiRunnerCompact.h:133`). The claim came from a stale `CLAUDE.md` table row, never from the code. `MAME_PARITY_AUDIT.md` § 2.2 corrected. | `rtc_pram_test` |
