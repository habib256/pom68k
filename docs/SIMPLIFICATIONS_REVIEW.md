...
| F7 | Floppy flux/PLL — ideal cells | **Read half CLOSED 2026-08-14 (steps 2-4b).** `SonyDrive` exposes a flux view (`nextFluxAfter`, opt-in `POM68K_FLUX_JITTER`); `Swim2` reads through a real `FluxPll` (snapshot v6); `Swim1`-ISM runs **MAME's real LS-pair/CSM/TSM engine** (`swim1.cpp:885-1233`) with the correction factors live — the parameter RAM became load-bearing, and the bite test lives in the suite (`P_MULT=0` starves the calibration, error `$08`, same track fails). No flag needed: on ideal edges the recovered bit stream is identical by construction, which every pre-existing floppy gate re-proves. The tail stays open, still symptom-free: a first-class flux *store* (off-rate written flux does not survive a canonical commit) and the `Iwm` READ path (hand-timed denibble stream, off limits without its etalons). `LLE_VS_HLE.md` § 1.3 carries the detail. | `swim2_media_test` +9 checks, `swim1_test` +7 — off-rate bites on both engines (jitter alone does not, as `flux_pll_test` predicted) |

**F2 to F6 are closed** (F2-F5 on 2026-08-13, F6 on 2026-08-14), each with
the gate named beside it. **F7's read half closed on 2026-08-14** (steps
2-4b of the flux plan, the LS-pair port included); what is left of the
seven is F7's tail — the flux store and the `Iwm` read path — both
symptom-free and behind the `TODO.md` test-depth pass.
