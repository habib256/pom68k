...
| F7 | Floppy flux/PLL — ideal cells | **Separator half CLOSED 2026-08-14 (steps 2-4a).** `SonyDrive` exposes a flux view (`nextFluxAfter`, opt-in `POM68K_FLUX_JITTER`), and `Swim2` + `Swim1`-ISM read through a real `FluxPll` (snapshot v6). No flag needed: on ideal edges the recovered bit stream is identical by construction, which every pre-existing floppy gate re-proves. The tail stays open, still symptom-free: a first-class flux *store* (off-rate written flux does not survive a canonical commit), the LS-pair correction port (now has PLL-recovered cells to discriminate), the `Iwm` READ path (hand-timed denibble stream, off limits without its etalons). `LLE_VS_HLE.md` § 1.3 carries the detail. | `swim2_media_test` +9 checks, `swim1_test` +1 — off-rate pair verified to bite (jitter alone does not, as `flux_pll_test` predicted) |

**F2 to F6 are closed** (F2-F5 on 2026-08-13, F6 on 2026-08-14), each with
the gate named beside it. **F7's separator half closed on 2026-08-14**
(steps 2-4a of the flux plan); what is left of the seven is F7's tail —
flux store, LS-pair port, `Iwm` read path — all symptom-free and behind
the `TODO.md` test-depth pass.
