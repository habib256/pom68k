...
|---|---|---|---|
| F1 | "PRAM persistence missing on 4 platforms" | **WITHDRAWN — the finding was false.** All twelve platforms declare `loadPram`/`savePram` and every runner wires the pair (the per-platform `GuiRunner*.h`, first pair `GuiRunnerToby.h:52`/`:244`; the compacts at `PlatformCompact.cpp:147`/`GuiRunnerCompact.h:162`). The claim came from a stale `CLAUDE.md` table row, never from the code. `MAME_PARITY_AUDIT.md` § 2.2 corrected. | `rtc_pram_test` |
...
| F4 | Egret/Cuda PC3 reset line = boot release only | **CLOSED 2026-08-13 on the six platforms that carried an Egret/Cuda LLE, and on the seventh — the Eclipse Q900/950 — on 2026-08-14 when it gained one** (V8, Sonora, VASP, RBV, Q605, Q630, Q700-eclipse; *not* eight: Centris has no Egret at all). Deferred by construction, as the trap demanded: the callback latches `restartPending_` and re-arms the overlay, and the CPU wrapper consumes it at a run boundary — the Duo's `MscCpu.cpp:44` contract. `CudaLle::hostReset()` factors the action out of the PC3 handler so the gate drives the shipped path, not a replica. | `cuda_restart_test` ✓ (30 checks, both flavours, three bindings) |
...
| F6 | Duo input through the ADB cell instead of the PMU matrix **+** PG&E NVRAM not persisted | **CLOSED 2026-08-14.** NVRAM persistence shipped with the 37th profile (`MscMemory.h:163-168`, `MscMemory.cpp:143-176`, `$91` power-flag scrub included); the matrix keyboard landed 2026-08-13 and the **trackball** on 2026-08-14 (`PgePmu`, quadrature counters `$14`-`$16` **latched at 60 Hz** — drained on read instead, a firmware poll that reads a register twice races itself and half the directions silently stop working). Nothing arrives on the ADB cell, which is what an *external* Duo keyboard/mouse would use: measured, the guest's own Mouse global never moves on that route, so the A/B lever was retired on 2026-08-25. The original ordering advice — do it *before* the `kProfiles` row — was overtaken: the Duo shipped as a GUI profile on 2026-08-06 anyway. | `duo_soak/persist_etalon` ✓ (the persist leg types Cmd-N *and* steers the pointer into the Finder's Special menu) |
| F7 | Floppy flux/PLL — ideal cells | **Read half CLOSED 2026-08-14 (steps 2-4b).** `SonyDrive` exposes a flux view (`nextFluxAfter`, opt-in `POM68K_FLUX_JITTER`); `Swim2` reads through a real `FluxPll` (snapshot v6); `Swim1`-ISM runs **MAME's real LS-pair/CSM/TSM engine** (`src/devices/machine/swim1.cpp:885-1233`) with the correction factors live — the parameter RAM became load-bearing, and the bite test lives in the suite (`P_MULT=0` starves the calibration, error `$08`, same track fails). No flag needed: on ideal edges the recovered bit stream is identical by construction, which every pre-existing floppy gate re-proves. **Tail CLOSED the same day (steps 5-6).** Flux became the *medium*, so a controller's own write rate lands on the disk instead of being quantized onto the media cell grid — and a commit no longer re-lays the track canonically, nor heals a write whose CRC failed. The `Iwm` READ path is MAME's window state machine (`src/devices/machine/iwm.cpp:398-455`), which cost what it was always going to cost: `disk_boot_etalon` + `lcii_floppy_etalon` re-proved. The gap4 became written self-sync on the way, because a transition-free arc gives a cell engine nothing to re-centre on. `LLE_VS_HLE.md` § 1.3 carries the detail and the three simplifications that stayed. | `swim2_media_test` +9 then +11 more, `swim1_test` +7 then +7, new `iwm_read_test`; both boot etalons green. Off-rate bites on both engines (jitter alone does not, as `flux_pll_test` predicted), and reinstating the canonical re-lay fails exactly the four medium checks |

**All seven are closed**, each with the gate named beside it: F2-F5 on
2026-08-13, F6 and F7 on 2026-08-14 — F7 in one day, its read half (steps
2-4b, the LS-pair port included) in the morning and its tail (steps 5-6,
the flux store and the `Iwm` read path) after. This list has nothing left
on it; the next depth comes from the `TODO.md` test-depth pass, not from
here.
