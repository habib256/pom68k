...
| What changed and **why** — dated entries, the project's real design record | `CHANGELOG.md` (255 entries; **everything that happened to one subsystem** → `CHANGELOG_INDEX.md`, generated by `tools/changelog_index.py`) |
...
- **JIT**: `jit/auto` is the conformant default on 68040 **and, since
  2026-08-18, on 68030**. AArch64 now resolves the 030 to native a64 with
  score 64; x86-64 keeps the portable `threaded` path. The C.5 flip
  to the x64 generator is **written but BLOCKED** (2026-08-19): `auto` now
  consults each backend's `caps().autoFamilies` SPEED declaration,
  separate from the `guestFamilies` correctness one, and the 030
  generator measures **−12 %** against `threaded` at the bench's default
  budget (`docs/JIT_BRINGUP.md` § C.4sexies) — but the first `-L m030`
  run under the flip found **the four IIsi gates in SIGSEGV ~5 s in**
  while the 030 platforms around them boot the generator green, and the
  `jit_*` 030 gates turn out to pin the engine, not the backend, so the
  generator had never executed the IIsi at all (§ C.4septies, parked
  with reproducer — D.1-4 red, no x64 flip commit until it is fixed).
  AArch64 independently cleared its lockstep, platform and fixed-budget
  evidence on 2026-08-20. `POM68K_JIT_BACKEND=x64|a64` still selects generated code on
...
29 patch groups, 391 marked lines, 80 `pom*` extension identifiers — a re-sync would be a port,
