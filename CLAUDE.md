...
| What changed and **why** — dated entries, the project's real design record | `CHANGELOG.md` (245 entries; **everything that happened to one subsystem** → `CHANGELOG_INDEX.md`, generated by `tools/changelog_index.py`) |
...
- **218 CTest gates** (`ctest -N` 2026-08-17: 106 `unit`, 9 `smoke`, 37 `jit`,
  51 `m040`, 112 `etalon`, of which 12 `etalon-core` — one profile per
  ...
  **host-conditional by two**: the AArch64 lockstep pair is registered only on
  an AArch64 host with the native backends, so an x86-64 configure sees 216.
...
ctest                        # 218 gates, ~4h (asset-dependent ones soft-skip)
...
ctest -L jit                 # 37 gates;  -L m040 = 51, the 68040 family
...
uncalibrated on every host** — sweep it with `tools/measure_gate_ram.py -L
etalon --fail-on-skip` where the images live. Measured on x86-64: the whole
runnable registry fits in one 256 MiB slot; `asset-none` 3.08 s → 0.80 s at
`-j4`, `unit` 9.45 s → 5.32 s, verdicts identical.
