...
| What changed and **why** — dated entries, the project's real design record | `CHANGELOG.md` (250 entries; **everything that happened to one subsystem** → `CHANGELOG_INDEX.md`, generated by `tools/changelog_index.py`) |
...
- **JIT**: `jit/auto` is the conformant default on 68040 **and, since
  ...
  **×1.18** on the LC II (17.90 s → 15.14 s at a fixed budget, one
  ...
  opt-in; the interpreter remains the explicit accuracy oracle, now with one
  `interp_*_boot_etalon` per 68030 platform as well. The remaining
  conformance gap and its plan: `docs/JIT_BRINGUP.md`.
