...
| What changed and **why** — dated entries, the project's real design record | `CHANGELOG.md` (263 entries; **everything that happened to one subsystem** → `CHANGELOG_INDEX.md`, generated by `tools/changelog_index.py`) |
...
- **226 CTest gates** (`ctest -N` 2026-08-21: 108 `unit`, 9 `smoke`, 39 `jit`,
  ...
  is **host-conditional by four**: the AArch64 lockstep pair is registered
  only on an AArch64 host with the native backends, and the x64 030
  experimental + alignment locksteps only on x86-64 — so either host's
  configure sees 224.
...
ctest                        # 226 gates, ~4h (asset-dependent ones soft-skip)
ctest -L unit                # 108 legacy non-etalon gates
...
ctest -L jit                 # 39 gates;  -L m040 = 51;  -L m030 = 53
