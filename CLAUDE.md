...
34 profiles → 20 `MachineKind` values → **12 platform implementations**
(the machine table below; the 12th, MSC, has no GUI profile yet).
`SnapMachine` in `src/SaveStateMachines.h` carries the matching 34 tags.
...
- **Save states**: all 11 machine families, GUI-wired.
- **JIT**: second engine, off by default (see the table below).
- The **PowerBook Duo 230** (MSC + PG&E, platform #11) boots the Finder under
  `duo230_boot_etalon` but is **not** a `kProfiles` row yet — house rule: a GUI
  profile is earned by a Finder cell *plus* the GUI/save-state wiring
  (`docs/DUO_BRINGUP.md`, `TODO.md` § 7). Same for the Quadra 900/950, whose
  platform is in `Q700Memory` but whose boot is still open (`TODO.md` § 0).
- Next: `TODO.md` § *New machine profiles* (the IIfx landed 2026-08-01 as the
  34th profile; ROMs on hand: Quadra 900/950), § *LLE fidelity*,
...
ctest                        # 136 gates, ~2h30 (asset-dependent ones soft-skip)
...
| Platform | Profiles (34) | Files | Gates | Source |
|---|---|---|---|---|
...
| **Discrete 040** (Mac II front end + Quadra back end) | Quadra 700 (`Q700Memory::Model` also carries the Q900/950 "Eclipse" front end — POSTs, boot still open, `TODO.md` § 0) | `Q700Memory.*`, `Q700Cpu.*`; SCSI through DAFB's own TurboSCSI cell | `q700_boot_etalon` (argument `q700` / `q900` / `q950`) | `DEV.md` § *Discrete-040 platform* |
