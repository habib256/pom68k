...
| What changed and **why** — dated entries, the project's real design record | `CHANGELOG.md` (230 entries; **everything that happened to one subsystem** → `CHANGELOG_INDEX.md`, generated by `tools/changelog_index.py`) |
...
| **Périphériques LLE/HLE** — the per-device registry (mode, reason, dump loaded, paths searched) reported by the devices themselves, and the GUI window that renders it + stages a manual override applied by a relaunch. Makes the § 2 "never silent" fallback policy visible to a user who never sees stderr | `LleSession.h` (registry), `PeripheralWindow.*` (window), reports in the 7 `*Memory` classes + `AdbVia` | ✓ `peripheral_lle_test` (25 checks, `unit`); the window is compile-verified only | `DEV.md` § 6; `docs/LLE_VS_HLE.md` § 2 |
