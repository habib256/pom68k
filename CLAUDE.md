...
| What changed and **why** — dated entries, the project's real design record | `CHANGELOG.md` (239 entries; **everything that happened to one subsystem** → `CHANGELOG_INDEX.md`, generated by `tools/changelog_index.py`) |
...
- **208 CTest gates** (`ctest -N` 2026-08-14: 96 `unit`, 9 `smoke`, 29 `jit`,
  ...
  **host-conditional by two**: the AArch64 lockstep pair is registered only on
  an AArch64 host with the native backends, so an x86-64 configure sees 205.
...
ctest                        # 208 gates, ~4h (asset-dependent ones soft-skip)
ctest -L unit                # 96 gates — no ROM or disk image needed
ctest -L smoke               # 9 gates — one machine, both CPU engines
ctest -L etalon-core         # 12 gates — ONE profile per platform, 31 min
ctest -L jit                 # 29 gates;  -L m040 = 51, the 68040 family
