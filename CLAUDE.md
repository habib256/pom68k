...
| What changed and **why** — dated entries, the project's real design record | `CHANGELOG.md` (233 entries; **everything that happened to one subsystem** → `CHANGELOG_INDEX.md`, generated by `tools/changelog_index.py`) |
...
| **IWM + Sony 3.5" 800K GCR** | `Iwm.h/.cpp`, `SonyDrive.h/.cpp` | M5 ✓; write engine + GCR write-back (`iwm_write_test`). **The READ path is MAME's real cell engine since 2026-08-14** (flux plan step 6, the last one): a window state machine re-centring on every transition over the flux store, replacing a fixed 128-cycle nibble cadence — `iwm_read_test`, and `disk_boot_etalon` + `lcii_floppy_etalon` as the price of touching the hand-timed denibble path. `SonyDrive` is a **flux** store now (step 5): the medium keeps the times a controller wrote, not a re-lay on its own cell grid | MAME `iwm.cpp`/`ap_dsk35.cpp`/`floppy.cpp` |
