...
| What changed and **why** — dated entries, the project's real design record | `CHANGELOG.md` (226 entries; **everything that happened to one subsystem** → `CHANGELOG_INDEX.md`, generated by `tools/changelog_index.py`) |
...
- **204 CTest gates** (`ctest -N` 2026-08-14: 94 `unit`, 9 `smoke`, 29 `jit`,
  49 `m040`, 110 `etalon`, of which 12 `etalon-core` — one profile per
  ...
  **host-conditional by two**: the AArch64 lockstep pair is registered only on
  an AArch64 host with the native backends, so an x86-64 configure sees 202.
...
ctest                        # 204 gates, ~4h (asset-dependent ones soft-skip)
