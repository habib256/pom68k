...
| What changed and **why** — dated entries, the project's real design record | `CHANGELOG.md` (253 entries; **everything that happened to one subsystem** → `CHANGELOG_INDEX.md`, generated by `tools/changelog_index.py`) |
...
- **225 CTest gates** (`ctest -N` 2026-08-18: 107 `unit`, 9 `smoke`, 38 `jit`,
  ...
  scope and tier dimensions; `performance_budgets.tsv` pins policy by
  workload, guest family and host. Both native CI jobs publish the same
  `pom68k.jit.metrics.v1` artifact rather than reducing the proof to a CTest
  exit status.
- Last FULL run **206/206 green, 2026-08-16** — the whole registry on this
  host, 94/94 `-LE etalon` + 112/112 `-L etalon`, 4 h 30 sequential, on a
  **tree rebuilt from scratch first** (`make -j4`, exit 0, no 0-byte
  binaries). The same tiers that morning were **102/112**: ten reds in five
  unrelated causes, after nine days without a whole-suite run
...
- **JIT**: `jit/auto` is the conformant default on 68040 **and, since
  2026-08-18, on 68030** — where it resolves to `threaded`. The C.5 flip
  ...
  with reproducer — D.1-4 red, no flip commit until it is fixed).
  AArch64 keeps `threaded` on 030 regardless (uncharge port + bench win
  pending). `POM68K_JIT_BACKEND=x64|a64` still selects generated code on
...
| **SWIM1** (IWM + ISM, 1.44 MB MFM) | `Swim1.h/.cpp` | ✓ `swim1_test`; ISM reads run MAME's real LS-pair/CSM/TSM engine — correction factors live, param RAM load-bearing, `P_MULT` bite pair in the suite (2026-08-14) | MAME SWIM (`swim1.cpp:885-1233`) |
