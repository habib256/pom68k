...
| **Save states** (archive core: one `visit<Ar>()` per class drives save AND load; chunked container, zero-run codec). Callbacks/pointers **re-bound not serialized**, caches **flushed**, disk images stay on the host (`ScsiDisk` copy-on-first-write log) | `SaveState.*`, `SaveStateMachines.*`, `MoiraSnapshot.h`, `visit()` in each device | LC II tree ✓ 2026-07-30 (20 classes); `savestate_test`, `savestate_v8_test` (both `unit`). **No GUI hook, no boot etalon, other 9 machines pending** — `TODO.md § C` | — |
