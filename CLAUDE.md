...
| What changed and **why** — dated entries, the project's real design record | `CHANGELOG.md` (205 entries; **everything that happened to one subsystem** → `CHANGELOG_INDEX.md`, generated by `tools/changelog_index.py`) |
...
- **183 CTest gates** (`ctest -N` 2026-08-12: 92 `unit`, 9 `smoke`,
...
| **GUI ↔ machine-thread contract** — thread, command queue, framebuffer double buffer, status atomics, save-state slot, JIT gauges. One CRTP host for all 12 platforms since 2026-08-09 (six copies before, 1 671 l. → 681 + a 383-l. header); platforms supply `emulateQuantum`/`drainAudio`/`renderFrame`/`publishStatus` only | `MachineHost.h`, `SaveStateSlot.h`; the six `*Machine` structs in `main.cpp` | ✓ `machinehost_test` (33 checks, `unit`). **The GUI layer above it is compile-verified only** — no gate opens a window | `DEV.md` § 6 |
...
23 patch groups, 358 marked lines, 50 `pom*` hooks — a re-sync would be a port,
