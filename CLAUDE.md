...
| What changed and **why** — dated entries, the project's real design record | `CHANGELOG.md` (231 entries; **everything that happened to one subsystem** → `CHANGELOG_INDEX.md`, generated by `tools/changelog_index.py`) |
...
- **207 CTest gates** (`ctest -N` 2026-08-14: 95 `unit`, 9 `smoke`, 29 `jit`,
  ...
  **host-conditional by two**: the AArch64 lockstep pair is registered only on
  an AArch64 host with the native backends, so an x86-64 configure sees 204.
...
ctest                        # 207 gates, ~4h (asset-dependent ones soft-skip)
ctest -L unit                # 95 gates — no ROM or disk image needed
...
| **SWIM2 + SuperDrive** | `Swim2.*`, `SonyDrive.*`, `FluxPll.h` | ✓; LLE cell engines (MFM CRC, rotation) gated; reads through a real FluxPll data separator over `SonyDrive`'s flux view — 12 % jitter + ±8 % off-rate media gated (`docs/LLE_VS_HLE.md` § 1.3) | MAME SWIM2 + `fdc_pll` |
