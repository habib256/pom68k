...
- **how a lockstep can report millions of identical checkpoints and still prove nothing** → [2026-08-06 (night) — The cycle-exact lockstep…](#2026-08-06-lockstep-68000)
- **which CPU wrappers carry a JIT engine, and what each is worth** → [2026-08-06 (late night) — The IIfx closes the set…](#2026-08-06-jit-iifx)
- **why the compacts' VBL comes from `MacFrameClock`, not `MacMemory::tick()` — and what that breaks in a harness** → [2026-08-06 (night) — The cycle-exact lockstep…](#2026-08-06-lockstep-68000)
...
use were already there — no Moira work at all — and its map is the **easiest
the window has met**: the IIfx is 32-bit clean, with no HMMU and no GLUE
24-bit remap (24-bit compatibility is the 030 PMMU's job), so what
`pomJitProbeCode` reports *is* the bus address. The Mac II board had to
refuse its own high-byte aliases and the V8 its bank remap; here there is
nothing to reconcile, and one RAM byte carries one name, so the write guard
needs no mirrored `note()` either.

The single thing worth care is the boot overlay, and it is the V8 case
exactly: on this machine a ROM-region **read** drops it (`rom_switch_r`,
`maciifx.cpp:169-186`). A windowed fetch performs no read. So while the
overlay is up `codeSpan` serves *nothing* — not low memory, and not the ROM
region whose read would drop it — because serving either would leave the map
latched in its boot state forever. `jitMapChanged()` fires at the drop.
...
that meant "something else is broken". Both are cases where the number on
the screen was not the question it appeared to answer, and both cost a
measurement to tell apart.

---

<a id="2026-08-06-lockstep-68000"></a>
## 2026-08-06 (night) — The cycle-exact lockstep, and the same trap twice in one day: a green gate that meant "nothing ran"

`jit_lockstep_68000_test`: two Mac Plus machines from the same ROM and the
same read-only floppy, one on each engine, compared at every checkpoint —
and unlike its 68040 twin the assertion that matters is **`clock`**, not the
register file. That is the whole reason the compact seam needed writing
(this morning's entry, § *the window is not free*): on `Core::C68020` a
windowed fetch cannot move the clock, on `Core::C68000` it can.

**The first version passed, and proved nothing.** It reported 2.5 M
checkpoints, 666 M guest cycles, 83 M instructions fetched through the
window. Then the negative control — deleting the contention charge from
`pomJitFetch000` — came back **green**, which is not a result, it is an
alarm. Instrumenting `codeSpan` by region gave the answer in one line:
`ramOK=0 ramNO=0 rom=83725`. Every windowed fetch over the whole "boot" came
from ROM. The guest had never executed a single instruction out of RAM.

The cause is specific to this family and it is in `MacFrame.h`: the 60.15 Hz
VBL is **not** generated by `MacMemory::tick()` the way every other
platform's is — `MacFrameClock::runFrame()` raises VIA CA1 itself, between
two `runUntil()` calls. A stepping harness built the obvious way, on
`runCycles()`, never delivers one. So the machine sat in early boot with the
overlay still up; low memory read ROM; `Cpu68k::applyContention` is RAM-only
and never fired; and a deleted charge was invisible. 666 million cycles of
looking busy.

Fixed by reproducing `runFrame()`'s shape inside the harness — run to the
vblank edge without overshooting it, raise CA1 on both machines, run out the
frame — and by making the gate **assert its own reach**: the overlay must
have dropped and the IWM must have been polled, or it fails. The numbers
moved accordingly:

| | before | after |
|---|---|---|
| window arms | 83 730 | **1 092 049** |
| arms refused | 6 | **47 155** |
| overlay at the end | up | **down** |
| floppy | untouched | **2 332 082 IWM polls, track 28** |

`track 28` is the same figure `system_boot_etalon` prints at the Finder, so
the comparison now covers the POST, the GCR read path, the System load and
the Finder — 646 M cycles, ~5 000 frames, coarse at 256 cycles then one
cycle for the last 100 000 checkpoints, so the sharpest comparison lands in
live Finder code rather than a boot loop.

**Both negative controls now bite**, which is the only thing that makes this
a gate: deleting the contention charge gives `DIVERGED after 982 402 steps —
CLOCK DIVERGENCE, delta −8`; deleting the trailing `SYNC(2)` gives
`DIVERGED after 0 steps`.

A `_blocks_test` variant forces the block path on (10 016 blocks compiled,
18.7 M instructions replayed, clock identical throughout). Recorded blocks
are sound on a cycle-exact core for a reason worth writing down: the
`threaded` backend replays through `pomJitExecOne()` — Moira's own handlers,
charging Moira's own cycles — and never replays the recorded
`Instr::cycles`. A code generator doing that arithmetic itself would be
wrong here, and no architectural comparison would catch it.

**The pattern, twice in twelve hours.** This morning the engine reported
itself ON while retiring zero instructions. Tonight a lockstep reported 2.5 M
identical checkpoints over code that never ran. Neither was a wrong answer;
both were *no* answer wearing a green light. The two habits that came out of
it — `POM68K_JIT_VERBOSE=1` printing retired/window/arms at teardown, and a
gate asserting its own reach before it reports success — are worth more than
the seam that occasioned them. One incidental fix along the way:
`MacMemory` had no `peek8()`, the side-effect-free read every other machine
map carries; `read8()` on this map clears VIA flags, advances the IWM and
latches the SCC, so there was no way to diff guest RAM without perturbing it.
...
both mode-5 cores suppress the tail refill, so `queue.irc` must preserve the
exact held lookahead/extension/displacement word at a block exit).
