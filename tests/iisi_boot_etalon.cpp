...
//
// Phase C gate: the Macintosh IIsi boots System 7.5 to the Finder on the
// new RBV machine (MAME maciici.cpp maciisi + rbv.cpp): 68030 @ 20 MHz,
// RBV RAM-based video (framebuffer = start of system RAM, Bt478 CLUT),
// Egret 344S0100 firmware LLE, NCR 5380 + pseudo-DMA, SWIM1, discrete
// ASC. The 512 KB ROM is $36B7FB6C. Soft-skips without the ROM or a
// bootable image.
//
...
int main() {
    std::string rom = find("roms/maciisi.rom");
    if (rom.empty())
        rom = find("roms/512KB ROMs/1990-10 - 36B7FB6C - Mac IIsi.ROM");
    std::string img = find("hdv/iisi-boot.vhd");
    ...
    const int64_t kFrame = RbvMemory::kCpuHz / 60;
    ...
    if (getenv("POM68K_DUMP")) {
        FILE* fp = fopen("iisi_screen.ppm", "wb");
        std::fprintf(fp, "P6\n%d %d\n255\n", W, H);
        for (int y = 0; y < H; y++)
            for (int x = 0; x < W; x++) {
                uint32_t p = fb[size_t(y) * W + x];
