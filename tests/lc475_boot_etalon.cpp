...
//
// Phase C gate: the Macintosh LC 475 / Performa 475 boots Mac OS 8.1 to
// the 640x480x8 Finder on the SAME Q605 machine as the Quadra 605, but
// with the LC 475 identity — model longword $A55A2221 and the 68LC040
// CPU (M68LC040 + soft 68882, the Finder-usable no-FPU path). The FF7439EE
...
int main() {
    ...
    std::printf("model ID $%08X (want $A55A2221)\n", id);
    if (id != 0xA55A2221u) {
        std::fprintf(stderr, "FAIL: model longword is not the LC 475\n");
        return 1;
    }
