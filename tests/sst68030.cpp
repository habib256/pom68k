...
int main(int argc, char** argv) {
    ...
    if (!fs::exists(dir)||!fs::is_directory(dir)) { std::fprintf(stderr,"SKIP: [sst68030] no data at '%s' (run oracle/fuzz/fuzz030.py)\n",dir.c_str()); return 0; }
    std::vector<fs::path> files;
    for (const auto& e : fs::directory_iterator(dir)) if (e.is_regular_file()&&e.path().extension()==".json") files.push_back(e.path());
    std::sort(files.begin(),files.end());
    if (files.empty()) { std::fprintf(stderr,"SKIP: [sst68030] '%s' holds no .json\n",dir.c_str()); return 0; }
    ...
    std::printf("[sst68030] %s: %ld/%ld across %ld file(s), %ld mmu-skipped%s\n",
                ...
                anyFail?"  <<< MISMATCH":"");
    if (filesRun==0) { std::fprintf(stderr,"SKIP: [sst68030] no files matched\n"); return 0; }
    return anyFail?1:0;
}
