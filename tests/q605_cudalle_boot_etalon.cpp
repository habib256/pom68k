...
//
// Cuda firmware-LLE boot gate (from the Q8 etalon): boot the FF7439EE Quadra 605 ROM and Mac OS 8.1 disk to the
...
int main() {
    // The Cuda firmware-LLE flavor of the boot etalon (blueprint step 3):
    // same assets, same Finder signature — but the ADB/PRAM/clock side runs
    // on the real 341S0788 firmware instead of the Egret HLE.
    ...
    if (!mem.cudaLleActive()) {
        std::printf("SKIP: needs roms/cuda/341s0788.bin for the firmware path\n");
        return 0;
    }
    ...
    std::printf("%s\n", ok ? "PASSED — Quadra 605 Finder on the REAL Cuda firmware" : "FAILED");
    return ok ? 0 : 1;
}
