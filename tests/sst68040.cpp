...
//
// SST040 replay harness — the error signal of the Phase-3 improve loop
// (TODO.md § Phase 3, Q2). Replays WinUAE-solo 68040 vectors produced by
// oracle/fuzz/fuzz040.py against Moira (Model::M68LC040 — the LC 475 /
// Quadra 605 CPU: integer + 040 MMU, no FPU) on the same flat 16 MB bus
// as tests/sst68040.cpp. Differences vs the 030 harness:
//
//   * SST040 format (oracle/fuzz/sst040.py) = SST030 + the eight 040 MMU
//     registers (urp040 srp040 tc040 itt0 itt1 dtt0 dtt1 mmusr040); the
//     030-only keys (crp/srp/tc/tt0/tt1/mmusr) stay 0 on an 040 run and
//     are still compared (an 040 must never touch them).
...
//
// Usage: sst68040 <dir> [--max N] [--only NAME,..] [--skip NAME,..]
...
struct CpuState {
    ...
    uint16_t mmusr = 0;
    uint32_t urp040 = 0, srp040 = 0, tc040 = 0;
    uint32_t itt0 = 0, itt1 = 0, dtt0 = 0, dtt1 = 0, mmusr040 = 0;
    uint8_t  stopped = 0;
    ...
    // Translation-enabled vectors (040 TC.E = bit 15); the predicate only
    // serves the --skip-translation debugging flag.
    bool needsTranslation() const { return (tc040 >> 15) & 1; }
};
...
public:
    TestCpu() : mem(1 << 24, 0) { setModel(moira::Model::M68LC040); }
...
void parseState(const char*& p, CpuState& st) {
    ...
    while (true) {
        ...
        if (*p=='"') {
            ...
            else if (key("urp040"))  st.urp040 = uint32_t(parseUint(p));
            else if (key("srp040"))  st.srp040 = uint32_t(parseUint(p));
            else if (key("tc040"))   st.tc040 = uint32_t(parseUint(p));
            else if (key("itt0"))    st.itt0 = uint32_t(parseUint(p));
            else if (key("itt1"))    st.itt1 = uint32_t(parseUint(p));
            else if (key("dtt0"))    st.dtt0 = uint32_t(parseUint(p));
            else if (key("dtt1"))    st.dtt1 = uint32_t(parseUint(p));
            else if (key("mmusr040")) st.mmusr040 = uint32_t(parseUint(p));
...
bool runVector(TestCpu& cpu, const Vector& v, bool checkCycles, std::string& why) {
    // Q2/Q4: the target CPU is the 68LC040 — NO FPU, ever. The corpus
    // still carries the (all-zero) FP keys because the shared SST030
    // serializer emits them; they are ignored: F2xx opcodes must take the
    // format $4 "FP disabled" exception, which an attached 68882 would
    // mask.
    cpu.setFPUModel(moira::FPUModel::NONE);
    ...
    // Q2: the 040 MMU registers load RAW (reg.* direct via the setters is
    // wrong here — the oracle state carries already-masked values, but a
    // fuzzer could seed unmaskable bits; the setters replicate the MOVEC
    // masks, which is exactly what oracle_set_state applies too).
    cpu.setURP040(v.initial.urp040);
    cpu.setSRP040(v.initial.srp040);
    cpu.setTC040(v.initial.tc040);
    cpu.setITT0(v.initial.itt0);
    cpu.setITT1(v.initial.itt1);
    cpu.setDTT0(v.initial.dtt0);
    cpu.setDTT1(v.initial.dtt1);
    cpu.setMMUSR040(v.initial.mmusr040);
    ...
    if (cpu.getURP040()!=v.fin.urp040) fail("URP040",cpu.getURP040(),v.fin.urp040);
    if (cpu.getSRP040()!=v.fin.srp040) fail("SRP040",cpu.getSRP040(),v.fin.srp040);
    if (cpu.getTC040() !=v.fin.tc040)  fail("TC040", cpu.getTC040(), v.fin.tc040);
    if (cpu.getITT0()  !=v.fin.itt0)   fail("ITT0",  cpu.getITT0(),  v.fin.itt0);
    if (cpu.getITT1()  !=v.fin.itt1)   fail("ITT1",  cpu.getITT1(),  v.fin.itt1);
    if (cpu.getDTT0()  !=v.fin.dtt0)   fail("DTT0",  cpu.getDTT0(),  v.fin.dtt0);
    if (cpu.getDTT1()  !=v.fin.dtt1)   fail("DTT1",  cpu.getDTT1(),  v.fin.dtt1);
    if (cpu.getMMUSR040()!=v.fin.mmusr040) fail("MMUSR040",cpu.getMMUSR040(),v.fin.mmusr040);
