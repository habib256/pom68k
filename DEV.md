...
LC II from reaching the Finder (CHANGELOG 2026-09-17 (tenth)). The GUI's display knobs,
...
`POM68K_SPEEDO_DISCOVER` (open, capture and cancel Speedometer's Benchmark
Mix, FPU and Color QuickDraw dialogs before the ordinary CPU phase),
...
pinned 1000000000), `POM68K_AFP_RENAME` (`<name>`: rename the server live
between the two connection cycles, so the second Chooser walk mounts it
under its new name — `q605_afp_rename_etalon`)
