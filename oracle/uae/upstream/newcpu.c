...
{
	/* POM68K: volatile — f is written between setjmp (TRY) and longjmp
	 * (THROW from a mmu030 access fault) and read in the CATCH block; a
	 * register-allocated f reverts to its pre-TRY value after longjmp
	 * (C11 7.13.2.1), which zeroed the restored CCR on every non-last-
	 * write MMU fault in release builds. See ../VENDOR.md. */
