...
**POM68K Disques**, the guest agent (`dev/scsiagent`, built with Retro68 and
shipped in every package as `share/POM68KDisques.bin`), mounts and unmounts
such disks on request. At launch POM68K installs it into the boot volume's
Startup Items — the `hdv/work/` clone for a reference volume, your own image
otherwise — and the Finder starts it with the desktop. The console says what
was done; `POM68K_NO_AGENT_AUTOSTART=1` leaves your System Folder alone.
