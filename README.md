...
ctest -j64                        # 239 gates, 20 min here (4 h 46 sequential).
...
The counts are the documented registry's (239 total); six lockstep gates
are host-conditional — three AArch64-only and three x86-64-only
(`jit_lockstep_030_x64_experimental_test`, `_packed_ccr_test` and
`_alignment_test`) — the AArch64 configure sees 236, and the
x86-64 configure sees 236 (x64: 111 unit / 8 smoke / 39 jit; AArch64:
111 / 9 / 39).
(`CMakeLists.txt`;
