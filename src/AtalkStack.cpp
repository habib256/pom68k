...
void AtalkStack::onGuestFrame(const uint8_t* d, size_t n) {
    ...
    if (type == kLlapEnq) {
        stats_.enqSeen++;
        // Inside AppleTalk appendix B sends lapACK directly from the receive
        // handler: it is a prompt inter-frame reply, not a fresh IDG dialog.
        if (dst == node_ && src == node_) {
            const uint8_t ack[3] = { src, node_, kLlapAck };
            if (sendAddressDefence) sendAddressDefence(ack, 3);
            else if (sendFrame) sendFrame(ack, 3);
        }
        return;
    }
