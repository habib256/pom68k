...
| `POM68K_JIT_ARM_BACKOFF` | `32` | single-stepped instructions after an `armWindow()` refusal before the next arm attempt (0-4096, `JitConfig.h:149`, consumed `JitEngine.cpp:74`) — the knob behind the "arm backoff 13.5 %" row of § C.4bis |
...
| `POM68K_JIT_RESTART_BASE` | `0` | reproducer knob (x64/030): admit the restartable-write family on the split BASE cost instead of the traced total — the ONE base admission still refused (JIT_BRINGUP § C.4sexies). Turning it on reproduces the coarse-budget lockstep divergence whose forensic is written there: NOT a cost bug — a peripheral-delivery alignment slip (identical clocks, devices and i-cache at the first slip; pc one instruction apart at a delivery point) inside the ROM's IRQ delay loops at `$40A07512` and `$40A08A6C`. Ships OFF until native pacing delivers at the same instruction boundaries as the fallback path |
| `POM68K_JIT_BSRW` | `0` | second reproducer knob for the SAME class: admit BSR.W (`$6100`) into the armed-charge exemption. Its linear charge is proved correct (`fetchWords=2`); the 120k lockstep still diverges at step 16 097, and the per-delivery trace shows the identical delivery-boundary slip. Ships OFF until the alignment chantier |
