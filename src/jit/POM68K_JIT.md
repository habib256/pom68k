...
| `POM68K_JIT_RESTART_BASE` | `0` | admission knob (x64/030): the restartable-write family on the split BASE cost instead of the traced total. Its historical coarse-budget divergence was the peripheral-phase class, CLOSED 2026-08-21 by the access-thunk clock bias (JIT_BRINGUP § C.4nonies); `jit_lockstep_030_x64_alignment_test` runs the 120k with this ON. Ships OFF because the flip is a speed decision awaiting its own D.1 evidence, behind the a64 alignment port |
| `POM68K_JIT_BSRW` | `0` | admission knob for BSR.W (`$6100`) into the armed-charge exemption. Charge proved correct (`fetchWords=2`); its step-16 097 divergence was the same peripheral-phase class, closed by the same fix and pinned by the same gate. Same flip conditions as `POM68K_JIT_RESTART_BASE` |
